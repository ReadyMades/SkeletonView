...
public extension UIView {
    ...
    @objc func showAnimatedHorizontalGradientSkeleton() {
        showSkeleton(skeletonConfig: SkeletonConfig(type: .gradient, colors: SkeletonAppearance.default.gradient.colors, animated: true, transition: .crossDissolve(0.25)))
    }
    
    @objc func hideSkeleton(reloadDataAfter reload: Bool = true) {
        flowDelegate?.willBeginHidingSkeletons(rootView: self)
        recursiveHideSkeleton(reloadDataAfter: reload, transition: .none, root: self)
    }
